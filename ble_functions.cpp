...
static bool isSupportedLegoManufacturerData(const NimBLEAdvertisedDevice* advertisedDevice) {
    ...
    return isWedoHub || isDuploTrain || isLegoSystemHub;
}
...
void WedoScanCallbacks::onResult(const NimBLEAdvertisedDevice* advertisedDevice) {
    ...
    bool isLwpHub = advertisedDevice->isAdvertisingService(NimBLEUUID(LWP_HUB_SERVICE_UUID)) || isSupportedLegoManufacturerData(advertisedDevice);
        ...
        if (wedo_name != nullptr) {
            if (advertisedDevice->haveName()) {
                ...
                if (deviceName == wedo_name) {
                    Serial.printf("Found our LEGO device: %s\n", wedo_name);
                    NimBLEDevice::getScan()->stop();
            ...
            // No name filter, connect to any supported LEGO device
            Serial.printf("Found a supported LEGO device\n");
            NimBLEDevice::getScan()->stop();
            pTargetDevice = advertisedDevice;
            doConnect = true;
        }
...
bool connectToWedoServer() {
    ...
    if (pWedoService != nullptr) {
        activeProtocol = BLE_PROTOCOL_WEDO;
        ...
        pWedoService = pWedoClient->getService(LWP_HUB_SERVICE_UUID);
        if (pWedoService == nullptr) {
        ...
        activeProtocol = BLE_PROTOCOL_LWP3;
        Serial.printf("Found LEGO Hub service\n");

        pHubCharacteristic = pWedoService->getCharacteristic(LWP_HUB_CHARACTERISTIC_UUID);
        if (pHubCharacteristic == nullptr) {
            Serial.printf("Failed to find LEGO Hub characteristic\n");
            pWedoClient->disconnect();
            return false;
        }

        pOutputCharacteristic = pHubCharacteristic;
        pInputCharacteristic = pHubCharacteristic;
        pSensorCharacteristic = pHubCharacteristic;

        if (pHubCharacteristic->canNotify()) {
            if (!pHubCharacteristic->subscribe(true, notifyCallback)) {
...
void writeBLECommand(int type, uint8_t* command, int size) {
    ...
    if (type == WEDO_INPUT) {
        pChar = activeProtocol == BLE_PROTOCOL_LWP3 ? pHubCharacteristic : pInputCharacteristic;
    } else if (type == WEDO_OUTPUT) {
        pChar = activeProtocol == BLE_PROTOCOL_LWP3 ? pHubCharacteristic : pOutputCharacteristic;
    }
